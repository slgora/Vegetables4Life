...
# Modified: 2025_12_13 - produce only 20km & 50km; add "resampled" to titles and output filenames
...
# Read in thinned Occurrences data at 20km
# only usable lat/long data (NAs and invalid lat/long removed)

#veglist1, 20km
occurrences_data <- read_csv("new_veg_list_data/Occurrence_data/veglist1/occurrences_20kmthinned_veglist1.csv")

#veglist2, 20km
occurrences_data <- read_csv("C:/Users/sarah/OneDrive/Desktop/GCCFP2/GCCFP2/new_veg_list_data/Occurrence_data/veglist2/occurrences_20kmthinned_veglist2.csv")
...
for (sz in names(grid_sizes)) {
  ...
  # --- MAP 1: Heat map of occurrence density (20km resampled) ---
  ...
  density_plot <- ggplot() +
    ...
    labs(
      title = sprintf("Global occurrence richness (%s, resampled)", sz),
      fill = "Occurrences"
    ) +
    theme_minimal()
  
  # Save MAP 1 as PDF and PNG (include "resampled" in filenames)
  out_pdf1 <- file.path(output_dir, sprintf("map01_occurrence_density_%s_resampled.pdf", sz))
  out_png1 <- file.path(output_dir, sprintf("map01_occurrence_density_%s_resampled.png", sz))
  ...
  # --- MAP 2: Heat map of species richness (20km resampled) ---
  ...
  richness_plot <- ggplot() +
    ...
    labs(
      title = sprintf("Global species richness (%s, resampled)", sz),
      fill = "Richness"
    ) +
    theme_minimal()
  
  # Save MAP 2 as PDF and PNG (include "resampled" in filenames)
  out_pdf2 <- file.path(output_dir, sprintf("map02_species_richness_%s_resampled.pdf", sz))
  out_png2 <- file.path(output_dir, sprintf("map02_species_richness_%s_resampled.png", sz))
